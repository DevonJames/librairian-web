...
export async function GET(request: Request) {
	try {
		...
		const transformedDocuments = documents.map((doc) => {
			...
			return {
				id: doc.id,
				// Include title - prefer doc.title, then fileName from JSON, then fallback
				title: doc.title || docJson?.fileName || null,
				...
				documentGroup: doc.documentGroup || null,
				processingStatus: docJson?.processingStage || null,
				// Include false redaction info if found
				falseRedactions: docJson?.falseRedactions?.found ? {
					found: true,
					pageCount: docJson.falseRedactions.pageCount,
					totalHiddenWords: docJson.falseRedactions.totalHiddenWords,
				} : null,
			};
		});
