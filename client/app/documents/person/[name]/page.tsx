...
export default function PersonProfilePage() {
  ...
  const handleSearchSubmit = async (e: React.FormEvent) => {
    ...
    setIsSearching(true);
    try {
      // First, try fetching from internal /api/docs/profile endpoint
      const profileResponse = await fetch(`/api/docs/profile/${decodedName}`);
      if (profileResponse.ok) {
        const profileData = await profileResponse.json();
        setEntityProfile(profileData);
      } else {
        // If internal API fails, try external search API
        const searchUrl = `${API_BASE_URL}/api/docs/search?text=${encodeURIComponent(searchQuery)}&person=${encodeURIComponent(decodedName)}&limit=50`;
      ...
      const docsResponse = await fetch(`${API_BASE_URL}/api/docs/search?person=${encodeURIComponent(decodedName)}&limit=50`);
      ...
      <div className="bg-gray-800 text-white p-4 shadow-md" style={{ flexShrink: 0 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', maxWidth: '100%' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
            <Link href="/documents" className="flex items-center text-sm hover:text-blue-300">
                          ...
                          {documentsToDisplay.map((doc) => (
                            ...
                            >
                              <div className="flex flex-col h-full">
                                <Link href={`/documents/${doc.id}`} className="grow">
                        ...
                        <div className="bg-gray-50 dark:bg-gray-700 rounded" style={{ padding: '0.75rem', height: '256px', overflow: 'auto' }}>
                          ...
                          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '0.5rem' }}>
                            {relatedNames.slice(0, 16).map((name, idx) => (
                              <Link key={`person-${idx}`} href={`/documents/person/${encodeURIComponent(name)}`} className="text-xs hover:underline hover:text-blue-600">
