...
async function repairIncorrectDocument(documentId: string): Promise<boolean> {
	try {
		...
		const mediaResponse = await fetch(
			`${API_BASE_URL}/api/docs/media?id=${documentId}&type=analysis&getLatestPageData=true`
		);
		...
		if (documentId && forceDataUpdate === true) {
			...
			while (attempt < maxRetries) {
				...
				try {
					...
					const mediaApiUrl = `${API_BASE_URL}/api/docs/media?id=${documentId.replace(
						/^\/+/,
						''
	...
	try {
		...
		try {
			...
			try {
				...
				const mediaResponse = await fetch(
					`${API_BASE_URL}/api/docs/media/status?id=${documentArchiveId}&collection=${effectiveDocType}`,
					{
						method: 'HEAD',
					}
				);
...
export async function GET(request: Request) {
	...
	try {
		...
		try {
			...
			const mediaResponse = await fetch(
				`${API_BASE_URL}/api/docs/media/status?id=${documentId}&collection=${effectiveDocType}`,
				{
					method: 'HEAD',
				}
			);
...
async function fetchDocumentAnalysis(
	...
	try {
		...
		const mediaApiUrl = `${API_BASE_URL}/api/docs/media?id=${cleanId}&type=analysis&getLatestPageData=true&collection=${documentType}`;
