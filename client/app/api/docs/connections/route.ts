...
export async function GET(request: NextRequest) {
	...
	// Collection filtering - can be a single collection or comma-separated list
	const collectionsParam = searchParams.get('collections');
	const collections = collectionsParam ? collectionsParam.split(',').map(c => c.trim().toLowerCase()) : [];
	...
	try {
		...
		switch (type) {
			...
			case 'timeline':
				if (documentId) {
					return await getTimelineData(documentId, collections);
				} else {
					// If no document ID is provided, return date statistics for all documents
					return await getDateStatistics(collections);
				}
			...
			case 'geo':
				if (documentId) {
					...
					return await getPlaceDocuments(null, collections);
				}
			...
			case 'places':
				...
				return await getPlaceDocuments(placeName, collections);

			case 'people':
				const personName = searchParams.get('name');
				return await getPersonDocuments(personName, collections);

			case 'objects':
				const objectName = searchParams.get('name');
				return await getObjectDocuments(objectName, collections);
...
async function getNetworkData(
	...
	console.log(
		`Getting network data for document ID: ${documentId} with ${layers} connection layers${collections.length > 0 ? `, filtered by collections: ${collections.join(', ')}` : ''}`
	);
	...
	if (allEntities.length > 0) {
		...
		// Build collection filter if collections are specified
		const collectionFilter = collections.length > 0 
			? { documentGroup: { in: collections } }
			: {};
...
async function getTimelineData(documentId: string, collections: string[] = []) {
	// Build collection filter if collections are specified
	const collectionFilter = collections.length > 0 
		? { documentGroup: { in: collections } }
		: {};
...
async function getDateStatistics(collections: string[] = []) {
	// Build collection filter for raw SQL
	const collectionCondition = collections.length > 0 
		? `AND "documentGroup" IN (${collections.map(c => `'${c}'`).join(', ')})`
		: '';

	// Get document counts by date
	const dateStats = await prisma.$queryRawUnsafe<
	...
	FROM "Document" 
	WHERE "earliestDate" IS NOT NULL ${collectionCondition}
...
async function getPlaceDocuments(placeName: string | null, collections: string[] = []) {
	// Build collection filter for raw SQL
	const collectionCondition = collections.length > 0 
		? `AND "documentGroup" IN (${collections.map(c => `'${c}'`).join(', ')})`
		: '';
	...
	if (!placeName) {
		...
		const placeStats = await prisma.$queryRawUnsafe<
	  ...
	  WITH place_data AS (
		SELECT 
		  ...
		  id
		FROM "Document"
		WHERE "allPlaces" IS NOT NULL ${collectionCondition}
	  )
	  SELECT 
		place,
	...
	// Build collection filter for Prisma query
	const collectionFilterPrisma = collections.length > 0 
		? { documentGroup: { in: collections } }
		: {};
...
async function getPersonDocuments(personName: string | null, collections: string[] = []) {
	// Build collection filter for raw SQL
	const collectionCondition = collections.length > 0 
		? `AND "documentGroup" IN (${collections.map(c => `'${c}'`).join(', ')})`
		: '';
	...
	if (!personName) {
		...
		const personStats = await prisma.$queryRawUnsafe<
	  ...
	  WITH person_data AS (
		SELECT 
		  ...
		  id
		FROM "Document"
		WHERE "allNames" IS NOT NULL ${collectionCondition}
	  )
	  SELECT 
		person,
		COUNT(DISTINCT id) as document_count
...
async function getObjectDocuments(objectName: string | null, collections: string[] = []) {
	// Build collection filter for raw SQL
	const collectionCondition = collections.length > 0 
		? `AND "documentGroup" IN (${collections.map(c => `'${c}'`).join(', ')})`
		: '';
	...
	if (!objectName) {
		...
		const objectStats = await prisma.$queryRawUnsafe<
	  ...
	  WITH object_data AS (
		SELECT 
		  ...
		  id
		FROM "Document"
		WHERE "allObjects" IS NOT NULL ${collectionCondition}
	  )
	  SELECT 
		object,
		COUNT(DISTINCT id) as document_count
